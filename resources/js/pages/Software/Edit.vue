...
const props = defineProps<{ software:any; vendors: {id:number; name:string}[] }>()
const form = useForm({ vendor_id: props.software.vendor_id, name: props.software.name, type: props.software.type, seats_total: props.software.seats_total, seats_used: props.software.seats_used, status: props.software.status, notes: props.software.notes })
const submit = () => form.put(`/software/${props.software.id}`)
</script>

<template>
  <Head title="Edit Software" />
  <AppLayout title="Edit Software">
    <ResourceToolbar title="Edit Software" :show-export="false" />
