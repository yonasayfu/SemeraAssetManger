...
const props = defineProps<{ vendors: any[]; products: any[]; assets: any[] }>()
const form = useForm({ type: 'lease', status: 'active', asset_id: null as number|null, vendor_id: null as number|null, product_id: null as number|null, start_at: '', end_at: '', amount_minor: null as number|null, currency: 'USD', notes: '' })
const submit = () => form.post('/contracts')
</script>

<template>
  <Head title="Add Contract" />
  <AppLayout title="Add Contract">
    <ResourceToolbar title="Add Contract" :show-export="false" />
