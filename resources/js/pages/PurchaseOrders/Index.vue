...
const props = defineProps<{ purchaseOrders: any; filters?: Record<string, any>; vendors?: Option[]; products?: Option[] }>()
...
const destroyItem = async (id: number) => {
  const accepted = await confirmDialog({
    ...
    message: 'This will permanently remove the purchase order.',
    confirmText: 'Delete',
    cancelText: 'Cancel',
  })
  if (!accepted) return
  ...
  <AppLayout title="Purchase Orders">
    ...
    <div class="mx-auto mt-4 w-full max-w-6xl px-4 print:hidden">
      <div class="flex flex-wrap items-center gap-2">
        <input v-model="search" type="text" placeholder="Search number or name" class="rounded-lg border border-transparent bg-white/80 px-3 py-2 text-sm text-slate-700 outline-none focus:border-indigo-300 focus:ring-0 dark:bg-slate-900/70 dark:text-slate-200" />
      ...
      <div class="overflow-hidden rounded-xl border border-slate-200/70 bg-white/80 shadow-sm dark:border-slate-800/60 dark:bg-slate-900/60">
        <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
          <thead class="bg-slate-50/80 dark:bg-slate-800/40">
            <tr>
              <th class="px-4 py-3 text-left font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-300">Number</th>
              ...
              <th class="px-4 py-3 text-left font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-300">Expected</th>
              <th class="px-4 py-3 text-left font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-300">Status</th>
              <th class="px-4 py-3 text-right font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-300">Total</th>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            <tr v-if="!purchaseOrders?.data?.length">
              <td colspan="6" class="px-4 py-8 text-center text-slate-500 dark:text-slate-400">No purchase orders found.</td>
            </tr>
            <tr v-for="po in purchaseOrders.data" :key="po.id" class="bg-white/70 dark:bg-slate-900/50">
              <td class="px-4 py-3 text-slate-700 dark:text-slate-200">
                <Link :href="`/purchase-orders/${po.id}/edit`" class="text-indigo-600 hover:underline dark:text-indigo-300">{{ po.number }}</Link>
              </td>
              <td class="px-4 py-3 text-slate-700 dark:text-slate-200">{{ po.name }}</td>
              <td class="px-4 py-3 text-slate-700 dark:text-slate-200">{{ po.vendor?.name || '—' }}</td>
              <td class="px-4 py-3 text-slate-700 dark:text-slate-200">{{ po.expected_delivery_at || '—' }}</td>
              <td class="px-4 py-3 text-slate-700 dark:text-slate-200">{{ po.status }}</td>
              <td class="px-4 py-3 text-right text-slate-700 dark:text-slate-200">{{ po.total_minor ? `$${(po.total_minor/100).toFixed(2)}` : '—' }}</td>
              <td class="px-4 py-3 text-right print:hidden">
                <div class="flex justify-end gap-2">
                  <Link v-if="can('purchase-orders.update')" :href="`/purchase-orders/${po.id}/edit`" class="inline-flex items-center rounded-md p-2 text-slate-500 transition hover:bg-slate-100 hover:text-indigo-600 dark:text-slate-300 dark:hover:bg-slate-800/70 dark:hover:text-indigo-300" title="Edit">
                    <Edit3 class="size-4" />
                    <span class="sr-only">Edit</span>
                  </Link>
                  <button v-if="can('purchase-orders.delete')" type="button" class="inline-flex items-center rounded-md p-2 text-red-500 transition hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/10" title="Delete" @click="destroyItem(po.id)">
                    <Trash2 class="size-4" />
