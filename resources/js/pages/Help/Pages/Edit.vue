...
const props = defineProps<{ page: { id: number; slug: string; title: string; excerpt?: string | null; content?: string | null; published?: boolean } }>();
...
const submit = () => {
  form.put(`/help/pages/${props.page.id}`);
};
</script>

<template>
  <Head :title="`Edit: ${props.page.title}`" />
  <AppLayout :breadcrumbs="[{ title: 'Help', href: '/help' }, { title: 'Pages', href: '/help/pages' }, { title: 'Edit', href: `/help/pages/${props.page.id}/edit` }]">
    <ResourceToolbar :title="`Edit: ${props.page.title}`" :show-create="false" />
