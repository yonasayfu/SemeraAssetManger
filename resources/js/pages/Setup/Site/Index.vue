...
const sites = computed<Site[]>(() => props.sites?.data ?? []);
const hasResults = computed<boolean>(() => sites.value.length > 0);
const paginationLinks = computed(() => props.sites?.links ?? []);
const paginationFrom = computed(() => props.sites?.meta?.from ?? 1);

const { show } = useToast();

const destroy = async (site: Site) => {
    const accepted = await confirmDialog({
        title: 'Delete site?',
        message: `This will permanently delete the site ${site.name} and all associated data.`,
        confirmText: 'Delete',
        cancelText: 'Cancel',
    });
    ...
    router.delete(`/setup/sites/${site.id}`, {
        onSuccess: () => show('Site deleted successfully.', 'danger'),
        onError: () => show('Failed to delete site.', 'danger'),
    });
};
...
const exportCsv = () => {
    ...
    window.open(`/setup/sites/export${query ? `?${query}` : ''}`, '_blank', 'noopener=yes');
};
</script>

<template>
    <Head title="Sites" />
    <AppLayout :breadcrumbs="[{ title: 'Setup', href: '' }, { title: 'Sites', href: '/setup/sites' }]">
        <div class="space-y-6">
            <ResourceToolbar
                title="Site directory"
                description="Manage physical locations where assets are stored or utilized."
                :create-route="can.create ? '/setup/sites/create' : undefined"
            ...
            <div class="overflow-hidden rounded-xl border border-slate-200/70 bg-white/70 shadow-sm dark:border-slate-800/60 dark:bg-slate-900/50">
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white/90 dark:divide-slate-800 dark:bg-slate-950/40">
                        <tr v-if="!hasResults">
                            <td colspan="4" class="px-6 py-8 text-center text-sm text-slate-500 dark:text-slate-300">
                                No sites match your filters yet. Add a new site to get started.
                            </td>
                        </tr>
                        <tr v-for="(site, index) in sites" v-else :key="site.id" class="hover:bg-slate-50/70 dark:hover:bg-slate-900/50">
                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                                {{ paginationFrom + index }}
                            </td>
                            <td class="px-6 py-4 text-sm font-medium text-slate-900 dark:text-slate-100">
                                <Link
                                    :href="`/setup/sites/${site.id}`"
                            ...
                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                                ...
                                <div v-if="site.city || site.state || site.postal_code" class="text-xs text-slate-500 dark:text-slate-400">
                                    {{ site.city }}{{ site.city ? ', ' : '' }}{{ site.state }} {{ site.postal_code }}
                                </div>
                            </td>
