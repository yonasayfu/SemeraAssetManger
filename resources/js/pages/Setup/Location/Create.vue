...
const submit = () => {
    form.post('/setup/locations', {
        onSuccess: () => {
            show('Location created successfully.', 'success');
        },
        onError: () => {
            show('Failed to create location.', 'danger');
        },
    });
};
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ title: 'Setup', href: '' }, { title: 'Locations', href: '/setup/locations' }, { title: 'Create', href: '/setup/locations/create' }]">
        <div class="space-y-6">
